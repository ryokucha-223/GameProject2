...
using UnityEngine.Video;//クリア演出映像を出すのに必要
...
{
    [SerializeField] GameObject jumpEff;//ジャンプ時のエフェクト
    private SpriteRenderer spriteRenderer;
    public float jumpPower = 10.0f;//ジャンプの力
    [SerializeField] float moveSpeed = 1.0f;//移動速度
    [SerializeField] float stepdis = 1.0f;
    [SerializeField] float stepmove = 2f;//ステップ距離
    [SerializeField] float backForce = 2.0f;//ノックバック
    //酸素関連
    [SerializeField] float oxyspeed = 0.000001f;//体力の自動消費量
    [SerializeField] public static float oxyval = 1.0f;//体力
    [SerializeField] float startoxy = 1.0f;//体力リセット用の奴上のと同じ数値に
    [SerializeField] float oxyheal = 0.5f;//体力の回復量（1.0を超えた分は無くなるものとする）
    [SerializeField] float oxyshot = 0.05f;//射撃で減る体力量
    [SerializeField] float oxybeam = 0.1f;//ビームで減る体力量
    [SerializeField] float oxystep = 0.02f;//ステップで消費する体力
    [SerializeField] float damage = 0.1f, thunder_damage = 0.1f, sickle_damage = 0.1f;//ダメージ受けて減る体力量
    ...
    [SerializeField] TextMeshProUGUI scoretext, cleartext, Gotext;//UI
    [SerializeField] GameObject getkeyflag;
    [SerializeField] Slider oxybar;//体力ゲージ
    [SerializeField] Image oxycolor;//体力ゲージの色
    ...
    [SerializeField] GameObject shotobj;//射撃の判定
    ...
    [SerializeField] GameObject bemefe;//射撃エフェクト
    [SerializeField] GameObject beamobj;//ビーム判定
    [SerializeField]
    ...
    bool isdead;//死亡判定
    bool GetKey;//キーアイテムの判定
    bool Getkagi;//鍵の判定
    bool isclear;//クリア判定
    private Vector3 targetPosition;
    private bool isStep = false;//ステップ中かどうか
    private bool canstep = true;//ステップできるかどうか
    //bool canmove;
